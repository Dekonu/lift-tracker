...
class ScheduledWorkout(Base):
    ...
    completed_workout_session_id: Mapped[int | None] = mapped_column(
        ForeignKey("workout_session.id"), nullable=True, index=True
    )
    ...
    )  # scheduled, completed, skipped, in_progress
    ...
    completed_session: Mapped["WorkoutSession"] = relationship(  # noqa: F821
        "WorkoutSession", foreign_keys=[completed_workout_session_id], init=False
    )
