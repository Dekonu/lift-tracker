...
async def create_workout_template(
    ...
    return WorkoutTemplateRead(**created) if isinstance(created, dict) else WorkoutTemplateRead.model_validate(created)
...
async def get_workout_templates(
    ...
    public_templates_data = await crud_workout_template.get_multi(
        db=db,
        offset=0,
        limit=10000,
        schema_to_select=WorkoutTemplateRead,
...
async def get_workout_template(
    ...
    return WorkoutTemplateRead(**template) if isinstance(template, dict) else WorkoutTemplateRead.model_validate(template)
...
async def apply_template_to_session(
    ...
    return {
        ...
        "note": "Templates currently store metadata only. Full implementation requires template exercise structure storage."
    }
